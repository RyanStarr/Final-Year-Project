...
def get_info(vm, depth=1):
   ...
   """
    ...
    vm_database(name, path, guest, description, power_state, ip_address, host_memory, guest_usage, assigned_storage, used_storage)
...
def get_system_specs(vm):
    ...
    host_database(num_cpu_cores, num_cpu_sockets, num_cpu_threads, cpu_mhz, cpu_model, memory_size, vendor, model, server_ip, vcenter_server_ip, num_of_nics)
...
def vm_database(name, path, guest, description, power_state, ipaddress, host_memory, guest_usage, assigned_storage, used_storage):
    # connect to database
    ...
    virtual_machine = [
        {"name": name, "path": path, "guest": guest, "description": description, "power_state": power_state,
         "ipaddress": ipaddress, "host memory": host_memory, "vm memory usage": guest_usage, "host storage assigned": assigned_storage, "vm storage used": used_storage}]
    x = mycol.insert_many(virtual_machine)


def host_database(num_cpu_cores, num_cpu_sockets, num_cpu_threads, cpu_mhz, cpu_model, memory_size, vendor, model, server_ip, vcenter_server_ip, num_of_nics):
    # connect to database
    myclient = pymongo.MongoClient("mongodb://localhost:27017/")
    ...
    system_specs = [
        {"CPU Cores": num_cpu_cores, "CPU Sockets": num_cpu_sockets, "CPU Threads": num_cpu_threads, "CPU Mhz": cpu_mhz, "CPU Model": cpu_model, "Memory Size": memory_size, "Vendor": vendor, "Model": model, "Server IP": server_ip, "vCenter IP": vcenter_server_ip, "Number of Network Interfaces": num_of_nics}]
...
def main():
   ...
   """
    ...
    si = SmartConnect(host="photon-machine",
                      user="administrator@esxi.local",
                      pwd="Networking101!",
                      port=443,
                      sslContext=None)
    if not si:
